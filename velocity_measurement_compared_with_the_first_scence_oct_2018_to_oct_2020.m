...
% % % This one will extract data from imagery pair between Scenes by
% comparing with the first scene.
...
outputfolder='C:\Users\Advice\OneDrive - Victoria University of Wellington - STUDENT\GISC 511\ImGraft\Velocity_Tiff';

% % list all the file names in the folder.

filenames = ls(strcat(workfolder, '\', '*.tif'));

% % remove some junk files from the name of imagery

% now there are 16 image names in the filenames variables.

% % Get date data from all imagery in that folder
  % NOTES: I will use similar technique here.
files = dir(workfolder); % get all file names in the "struct array" format.
files = files(3:118); % remove some junk names in that folder. 
...
% % Use for loop to find the displacement velocity and save it into a
  % georeference file every time it loops by paring image 1 with 1 + i. 
...
for i = 1:(length(filenames)-1) % we will loop from 1 to the second last number of imager (Pairing 1 and 1 + i)
   ...
   [A,x,y,Ia]=geoimread(fullfile(workfolder,filenames(1,1:12)));
   ...
   num_days = date_num(j) - date_num(1);
   ...
   geotiffwrite(strcat(outputfolder,'\', filenames(1,1:8), '_', filenames(j,1:8), '.tif'),Vn,R)
end
