...
% % % Thatsanee Sanouban % % %
% % % Try will use different spatial resolution and observe the results % % %
% % % Venus Satellite Imagery % % % 20 meter 
...
% % set working folder to the folder that contains all preprocessed imagery 2018 - 2020
  ...
  % This study area is Franz Josef, It will be abit different from Tasman
  % Glacier.
  
workfolder='C:\Users\Advice\OneDrive - Victoria University of Wellington - STUDENT\GISC 511\Study Areas\Tasman\Tif_UTM - Deresolution\Despatial-Resolution\20 meters';
outputfolder='C:\Users\Advice\OneDrive - Victoria University of Wellington - STUDENT\GISC 511\ImGraft\Tasman_Deresolution_Velocity\20 meters';
...
for i = 1:(length(files)-1) % we will loop from 1 to the second last number of imager (Pairing 1 and 1 + i)
   ...
   [pu,pv]=meshgrid(0:3:size(A,2), 0:3:size(A,1)); %pixel coordinated
   ...
   [du,dv,C,Cnoise,pu,pv]=templatematch(A,B,pu,pv,'TemplateWidth', 10, 'TemplateHeight',10,'SearchWidth', 20, 'SearchHeight', 20, 'showprogress',true,'method','OC');
   close all
   ...
   disp(i) % this is just to see the progress.
